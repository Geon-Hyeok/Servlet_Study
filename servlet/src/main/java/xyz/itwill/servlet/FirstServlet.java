...
// => name 속성 : 서블릿의 이름을 속성값으로 설정 - 속성을 생략하면 자동으로 클래스 이름을 서블릿의 이름으로 설정 
// => value 속성 : 서블릿을 요청하기 위한 URL 패턴을 속성값으로 설정 - value 속성외 다른 속성이 없는 경우 속성값만 설정 가능
...
public class FirstServlet extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		// 클라이언트에게 요청에 대한 실행결과를 저장하기 위한 파일형태 변경 및 응답파일을 생성하기 위한 출력 스트림을 반환받아 저장
		...
		Date now = new Date(); // 서버 플렛폼의 현재 날짜와 시간이 저장된 Date 객체 생성
